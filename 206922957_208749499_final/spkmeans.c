...
void printAr(double *ar, int size){
    ...
    char buff [100];
    double tmp_val;
    double zero_float = 0.0000; */
    for(;i < size; i++){
        if (i == size-1){
            suffix = '\n';
        } /*
        sprintf(buff, "%.4f", ar[i]);
        tmp_val = atof(buff);
        
s
        if (tmp_val == zero_float) {
            ar[i] = zero_float;
        } */
...
double ** jacobi(double **vectors, int dim){
    ...
    double ***memory = malloc(sizeof(double **) * 6);
    int memory_index = 0;
    ...
    eyeMat = eye(dim);
    memory[memory_index++] = eyeMat;

    if (matrix == NULL || matrixPrime == NULL || finalEigenvectors == NULL || rotationMatrix == NULL || tempEigenvectors == NULL || eyeMat == NULL){
    ...
    while (espilon > CONVERGENCE && iterNum <= 100){
        ...
        /* Code added by Nir, need to make sure it's noy buggy */
        if (maxValue == 0.0) {
            /*printf("MAX VAL IS %f   ",maxValue); */
            result = createBlockMatrix(sizeof(double), dim + 1, dim); 
            if (result == NULL){
                freeArrayOfBlocks(memory, memory_index);
                return NULL;
                }
            ...
            copyMatrix((result + 1), eyeMat, dim, dim);
            freeArrayOfBlocks(memory, memory_index);
            return result;
            }
